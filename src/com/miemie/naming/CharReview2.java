...
public class CharReview2 extends Activity implements OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mDB = Utils.openNameDatabase(this);
...
//        mResult = Utils.combineSToSet(temp, "@");
        ...
        for (int i = 0; i < mIds.length; i++) {
            ...
            mTVs[i * COLUMNS + 0].setOnClickListener(this);
            mTVs[i * COLUMNS + 0].setTag(Boolean.FALSE);
    ...
    public void onBackPressed() {
        mDialog = new ProgressDialog(CharReview2.this);
    ...
    private void updateView() {
        ...
        for (int i = 0; i < mIds.length; i++) {
            int index = mIndex * mIds.length + i;
            if (index < mSource.length) {
                String str = mSource[index];
                Cursor c = db.rawQuery(
                        "select strokes,spell,explain from signalName where signalNM=?",
                        new String[] {
                            str
                        });
                if (c == null) {
                ...
                c.moveToFirst();
                mTVs[i * COLUMNS].setText(str);
                StringBuilder sb = new StringBuilder("(");
                sb.append(c.getString(1));
                sb.append(",");
                sb.append(c.getInt(0));
                sb.append(")");
                mTVs[i * COLUMNS + 1].setText(sb.toString());
                mTVs[i * COLUMNS + 2].setText(c.getString(2));
                c.close();
            } else {
                mTVs[i].setText("");
            }
